...
// MatchSkills returns skills matching the prompt keywords, sorted by relevance.
func (cs *CodeSkillStore) MatchSkills(prompt string, maxResults int) []*CodeSkill {
	if cs == nil || len(cs.Skills) == 0 {
		return nil
	}
	...
	for _, s := range cs.Skills {
		...
		allKeywords := append([]string{}, s.Keywords...)
		allKeywords = append(allKeywords, strings.Fields(strings.ToLower(s.Description))...)
		allKeywords = append(allKeywords, strings.ToLower(s.Name))

		score := wordOverlap(words, allKeywords)
		if score > 0.3 {
	...
	result := make([]*CodeSkill, len(matches))
...
// FormatSkillsContext formats matching skills for injection into LLM context.
func (cs *CodeSkillStore) FormatSkillsContext(prompt string) string {
	matches := cs.MatchSkills(prompt, 5)
	if len(matches) == 0 {
		return ""
	}

	var b strings.Builder
	b.WriteString("\n\nAvailable code skills (use run_skill tool to execute):\n")
